...
export const Garden = class {
  constructor(canvas, counterHtml) {
    this.canvas = canvas;
    this.counterHtml = counterHtml;
      ...
      if (pointerInfo.type === BABYLON.PointerEventTypes.POINTERDOWN) {
        ...
        if (pick.hit) {
          ...
          if (pickedMesh.meshType === "fruit") {
            ...
            this.counterHtml.innerHTML = +this.counterHtml.innerHTML + 1;
  ...
  setCamera(cameraOptions) {
    this.camera.setTarget(cameraOptions.target);
    this.camera.upperRadiusLimit = cameraOptions.radius || CAMERA_RADIUS;
    this.camera.radius = cameraOptions.radius || CAMERA_RADIUS;
    this.camera.rebuildAnglesAndRadius();
  }

  setSky(skyOptions) {
    Object.keys(skyOptions).forEach(skyOption => {
      this.skybox.material[skyOption] = skyOptions[skyOption];
    });
  }

  setFog(fogOptions) {
    Object.keys(fogOptions).forEach(fogOption => {
      this.scene[fogOption] = fogOptions[fogOption];
    });
  }

  setGround(groundOptions) {
    if (groundOptions.diffuseTexture) {
      const diffuseTexture = new BABYLON.Texture(groundOptions.diffuseTexture.url, this.scene);
      diffuseTexture.uScale = groundOptions.diffuseTexture.uScale || 1;
      diffuseTexture.vScale = groundOptions.diffuseTexture.vScale || 1;
      this.ground.material.diffuseTexture = diffuseTexture;
    }
    if (groundOptions.bumpTexture) {
      const bumpTexture = new BABYLON.Texture(groundOptions.bumpTexture.url, this.scene);
      bumpTexture.uScale = groundOptions.bumpTexture.uScale || 1;
      bumpTexture.vScale = groundOptions.bumpTexture.vScale || 1;
      this.ground.material.bumpTexture = bumpTexture;
      this.ground.material.useParallax = true;
      this.ground.material.useParallaxOcclusion = true;
      this.ground.material.parallaxScaleBias = groundOptions.bumpTexture.parallaxScaleBias;
    }
    if (groundOptions.detailMap) {
      const detailMap = new BABYLON.Texture(groundOptions.detailMap.url, this.scene);
      detailMap.uScale = groundOptions.detailMap.uScale || 1;
      detailMap.vScale = groundOptions.detailMap.vScale || 1;
      this.ground.material.detailMap.isEnabled = true;
      this.ground.material.detailMap.diffuseBlendLevel = groundOptions.detailMap.diffuseBlendLevel;
      this.ground.material.detailMap.bumpLevel = groundOptions.detailMap.bumpLevel;
      this.ground.material.detailMap.roughnessBlendLevel = groundOptions.detailMap.roughnessBlendLevel;
      this.ground.material.detailMap.texture = detailMap;
    }
  }
  ...
  async setGarden(gardenObject, fruitList) {
    ...
    this.setCamera(gardenObject.cameraOptions);
    this.setSky(gardenObject.skyOptions);
    this.setFog(gardenObject.fogOptions);
    this.setGround(gardenObject.groundOptions);
    this.startRender();
  }
}
